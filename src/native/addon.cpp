...
Napi::Value DiscordSocialSDK::UpdateActivity(const Napi::CallbackInfo& info) {
    ...
    // Default activity type to Playing (0)
    ...
    if (activityObj.Has("type")) {
        activity.activityType = activityObj.Get("type").As<Napi::Number>().Int32Value();
    }
...
Napi::Value DiscordSocialSDK::Authorize(const Napi::CallbackInfo& info) {
    ...
    sdk_->Authorize([tsfn](const DiscordWrapper::AuthResult& result) {
        auto callback = [result](Napi::Env env, Napi::Function jsCallback) {
            ...
            resultObj.Set("refreshToken", Napi::String::New(env, result.refreshToken));
            resultObj.Set("expiresIn", Napi::Number::New(env, result.expiresIn));
